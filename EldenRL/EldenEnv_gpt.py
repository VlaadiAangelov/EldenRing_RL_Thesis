...
class EldenEnv(gym.Env):
    ...
    def check_for_second_phase(self):
        ...
        self.reward, self.death, self.boss_death, self.duel_won = self.rewardGen.update(frame, self.first_step, None, self.step_iteration) #perform eldenreward update to get the current status of the boss
    ...
    def step(self, action):
        ...
        self.reward, self.death, self.boss_death, self.duel_won = self.rewardGen.update(frame, self.first_step, None, self.step_iteration) #📍 2. Collect the reward based on the observation (reward of previous step)
